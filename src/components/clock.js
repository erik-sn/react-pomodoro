...
export default class Clock extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      audio: new Audio('/pomodoro/resources/beep.wav'),
  ...
  plusMinus(field, val) {
    const update = {};
    const newVal = parseInt(this.state[field], 10) + val;
    if (newVal >= 0) {
      update[field] = newVal;
  ...
  render() {
    const { start, mode, timer, active, labelCount, breakDuration,
      workDuration, blinkCount } = this.state;
    const labels = this.generateLabels(225, parseInt(start, 10), parseInt(labelCount, 10));
